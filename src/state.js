...
export const [activePreset, setActivePreset] = createSignal('DISCO INFERNO');
...
export async function triggerDrop() {
    await audio.init();
    audio.drop();
...
export function setFilterValue(v) {
    setFilter(v);
    if (audio.ctx)
        audio.setFilter(v);
}
export function setReverbValue(v) {
    setReverb(v);
    if (audio.ctx)
        audio.setReverb(v * 0.7);
}
...
export function toggleCell(track, step) {
...
export function clearCell(track, step) {
    const pats = seq.patterns;
    if (track === 'bass' || track === 'lead') {
